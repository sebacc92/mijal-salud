...
import { DIMENSION_COLORS } from "~/components/servicios/colors";

export const useAreaProtegidaContent = routeLoader$(async ({ cacheControl }) => {
  publicContentCache(cacheControl);
  return getPageContent("area-protegida");
});

// Acentos de color de los planes (clases literales para el JIT de Tailwind).
const PLAN_BADGE = {
  violet: "text-violet-600 bg-violet-50 border-violet-200",
  navy: "text-navy-800 bg-navy-100 border-navy-200",
};
const PLAN_BUTTON = {
  violet: "bg-violet-600 hover:bg-violet-700",
  navy: "bg-navy-900 hover:bg-navy-800",
};

export default component$(() => {
  const c = useAreaProtegidaContent().value;
      ...
      <section class="bg-navy-900 relative overflow-hidden py-20">
        ...
        <div class="relative container mx-auto px-6 lg:px-12 max-w-3xl text-center">
          <div class="inline-flex items-center gap-2 bg-violet-500/20 border border-violet-400/40 rounded-full px-4 py-2 mb-8">
            <span class="text-violet-200 text-sm font-body font-medium">{c.heroBadge}</span>
          </div>
          <h1 class="font-display text-h1 text-white mb-5">
            {c.heroTitle} <span class="text-violet-300">{c.heroTitleHighlight}</span>
          </h1>
      ...
      <section class="py-section bg-white">
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 class="font-display text-h2 text-navy-900 mb-6">{c.coberturaTitle}</h2>
              <ul class="space-y-4">
                {c.coberturaItems.map((item) => (
            ...
            <div class="bg-gris-50 rounded-3xl p-8 border border-gris-200">
              <h3 class="font-display font-bold text-navy-900 text-lg mb-5">{c.dimensionesTitle}</h3>
              <div class="space-y-4">
                {c.dimensiones.map((r) => (
                  <div key={r.rango} class="flex items-start justify-between gap-4 bg-white rounded-xl p-4 border border-gris-200">
                    ...
                    <span class={["text-xs font-body font-semibold px-3 py-1 rounded-full shrink-0", DIMENSION_COLORS[r.colorKey] ?? DIMENSION_COLORS.verde]}>{r.equipo}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="casos" class="py-section bg-gris-50">
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {c.casos.map((caso) => (
              <div key={caso.label} class="bg-white border border-gris-200 rounded-2xl p-6 text-center hover:border-violet-300 hover:shadow-card transition-all duration-300">
                <div class="text-4xl mb-3">{caso.emoji}</div>
                <p class="text-gris-700 font-body text-sm font-medium">{caso.label}</p>
              </div>
            ))}
          </div>
        ...
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          <div class="text-center mb-12">
            <div class="inline-flex items-center gap-2 bg-verde-500/10 border border-verde-500/20 rounded-full px-3 py-1 mb-4">
              <span class="text-verde-600 text-xs font-body font-semibold uppercase tracking-wider">{c.planesEyebrow}</span>
            </div>
            <h2 class="font-display text-h2 text-navy-900 mb-4">{c.planesTitle}</h2>
            <p class="text-gris-600 font-body text-body-lg max-w-xl mx-auto">{c.planesSubtitle}</p>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            {c.planes.map((plan, i) => {
              const accent = i === 0 ? "violet" : "navy";
              if (plan.recommended) {
                return (
                  <div key={plan.title} class="bg-navy-900 border-2 border-verde-500 rounded-3xl p-8 flex flex-col justify-between shadow-card relative transform lg:-translate-y-2 transition-all duration-300">
                    ...
                    <div>
                      <span class="text-xs font-bold text-verde-400 bg-verde-900/50 border border-verde-800 px-3 py-1 rounded-full uppercase tracking-wider font-body">{plan.badge}</span>
                      <h3 class="font-display font-bold text-xl text-white mt-4 mb-2">{plan.title}</h3>
                      <p class="text-white/60 text-sm font-body mb-6">{plan.desc}</p>
              ...
              return (
                <div key={plan.title} class="bg-gris-50 border border-gris-200 rounded-3xl p-8 flex flex-col justify-between shadow-card hover:shadow-card-hover transition-all duration-300">
                  <div>
                    <span class={["text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider font-body border", PLAN_BADGE[accent]]}>{plan.badge}</span>
                    <h3 class="font-display font-bold text-xl text-navy-900 mt-4 mb-2">{plan.title}</h3>
                    <p class="text-slate-500 text-sm font-body mb-6">{plan.desc}</p>
                  ...
                  <div class="mt-8">
                    <a href={WHATSAPP_EMPRESAS} target="_blank" rel="noopener noreferrer"
                      class={["block text-center text-white font-display font-semibold text-sm py-3.5 rounded-xl transition-colors cursor-pointer", PLAN_BUTTON[accent]]}>
                      {plan.ctaLabel}
                    </a>
                  </div>
                </div>
      ...
      <section class="py-section bg-gris-50 border-t border-gris-150">
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div class="inline-flex items-center gap-2 bg-verde-500/10 border border-verde-500/20 rounded-full px-3 py-1 mb-4">
                <span class="text-verde-600 text-xs font-body font-semibold uppercase tracking-wider">{c.zonaEyebrow}</span>
              </div>
              <h2 class="font-display text-h2 text-navy-900 mb-6">{c.zonaTitle}</h2>
              <p class="text-gris-600 font-body text-sm leading-relaxed mb-6">{c.zonaText}</p>

              <div class="space-y-4">
                {c.zonaPoints.map((pt) => (
                  <div key={pt.title} class="flex items-start gap-3 bg-white p-4 rounded-xl border border-gris-200">
                    <span class="text-2xl">{pt.emoji}</span>
                    <div>
                      <h4 class="font-display font-bold text-navy-900 text-sm">{pt.title}</h4>
                      <p class="text-gris-500 text-xs font-body mt-0.5">{pt.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
...
export const head: DocumentHead = ({ resolveValue }) => {
  const c = resolveValue(useAreaProtegidaContent);
  return {
    title: c.seo.title,
