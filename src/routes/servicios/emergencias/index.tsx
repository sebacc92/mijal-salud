...
export const useEmergenciasContent = routeLoader$(async ({ cacheControl }) => {
  publicContentCache(cacheControl);
  return getPageContent("emergencias");
});

export default component$(() => {
  const c = useEmergenciasContent().value;
    ...
    <main class="pt-24">
      <section class="bg-red-900 relative overflow-hidden py-24">
        ...
        <div class="relative container mx-auto px-6 lg:px-12 max-w-4xl text-center">
          <div class="inline-flex items-center gap-2 bg-red-500/20 border border-red-400/40 rounded-full px-4 py-2 mb-8">
            ...
            <span class="text-red-200 text-sm font-body font-medium">{c.heroBadge}</span>
          </div>
          <h1 class="font-display text-h1 text-white mb-5">
            {c.heroTitle} <span class="text-red-300">{c.heroTitleHighlight}</span>
          </h1>
      ...
      <section class="py-section bg-white">
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          <div class="text-center mb-12">
            ...
            <p class="text-gris-600 font-body text-body-lg max-w-xl mx-auto">{c.cuandoSubtitle}</p>
          </div>
      ...
      <section class="py-section bg-gris-50">
        <div class="container mx-auto px-6 lg:px-12 max-w-5xl">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 class="font-display text-h2 text-navy-900 mb-6">{c.traemosTitle}</h2>
              <ul class="space-y-4">
                {c.traemosItems.map((item) => (
      ...
      <section class="py-14 bg-red-600">
        <div class="container mx-auto px-6 text-center">
          <h2 class="font-display font-black text-white text-3xl mb-3">{c.ctaFinalTitle}</h2>
          <p class="text-white/80 font-body mb-6">{c.ctaFinalSubtitle}</p>
...
export const head: DocumentHead = ({ resolveValue }) => {
  const c = resolveValue(useEmergenciasContent);
  return {
    title: c.seo.title,
